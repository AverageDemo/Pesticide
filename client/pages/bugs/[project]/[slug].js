  ...
  return (
    <Layout
      breadcrumb={[
        ...
        <Link href={`/projects/${projectObj.slug}`} key={projectObj.name}>
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
        <div className="px-4 py-5 sm:px-6">
          ...
          <p className="mt-1 max-w-2xl text-sm text-gray-500">
            Created on {moment(bug.date).format("LLLL") + " EST"} - by{" "}
            {bug.author.name}
          </p>
          {bug.date !== bug.dateUpdated && (
            <p className="mt-1 max-w-2xl text-sm text-gray-500">
              Last updated on {moment(bug.dateUpdated).format("LLLL") + " EST"}
            </p>
          )}
        </div>
          ...
          <dl>
            ...
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <span
                  className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full text-white ${
                    Object.values(severityOptions)[bug.severity]
                  }`}
                >
            ...
            <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                {bug.assigned ? bug.assigned.name : "Not Assigned"}
              </dd>
            </div>
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            <div className="px-4 py-3 bg-white flex justify-between sm:px-6 border-t border-gray-100">
              <div>
                {user._id === bug.author._id ? (
                  <Link href={`/bugs/${projectObj.slug}/${bug.slug}/edit`}>
              ...
              <div>
                {(user.role > 1 || user._id === bug.author._id) && (
                  <button
                ...
                {bug.status < 3 && user.role > 1 ? (
                  ...
                  //     Assign to user
                  // </button>
                  <SelectUser bug={bug} token={token} people={people} />
                ) : bug.status === 1 ? (
                  <button
            ...
            >
              <div className="px-4 py-2 sm:px-6 flex justify-between">
                <p className="mt-1 max-w-2xl text-sm text-gray-600">
                  <span className="font-semibold">{ctx.author.name}</span>
                  {` posted ${moment(ctx.date).fromNow()}`}
                </p>

                {user && (user.role > 1 || user._id === ctx.author._id) && (
                  <p className="mt-1 max-w-2xl text-sm text-gray-600">
                    <button
                      onClick={() => handleDeleteComment(ctx._id)}
